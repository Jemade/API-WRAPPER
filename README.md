...
1. **Vendor Decoupling**: Clients consume a single, normalized API format regardless of whether the underlying model is hosted by OpenAI or Anthropic.
...
- **Provider Abstraction**: Unified adapter interface supporting OpenAI (`gpt-4o`, `o1`, etc.) and Anthropic (`claude-3-5-sonnet`, etc.) with normalized response shapes.
